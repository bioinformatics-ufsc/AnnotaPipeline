...
for gene in gff:
	...
	trans[gene_features[-1]] = {"scaff": str(gene_features[0]), "start": str(gene_features[3]), "end":str(gene_features[4]),"strand":str(gene_features[6])}
...
for sequence in fasta:
	...
	id = sequence[0]
	try:
		out.write(">" + str(id) + " | Organism: " + str(args.org)  +" | Location: " + str(trans[id]['scaff']) + " | Start: " + str(trans[id]['start']) + " | End: " + str(trans[id]['end']) + " | Strand: " + str(trans[id]['strand']) + " | Description: " + str(ant[id]) +"\n")
		out.write(str("\n".join(sequence[1:])))
	except:
		ids_warn.append(id)		
		pass
